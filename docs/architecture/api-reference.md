...
| GET | `/api/tournaments`, `/api/tournaments/[tournamentId]/standings` | Session (depuis le 2026-09-16 ; auparavant **aucune**) | ✅ Pertinent | Liste des tournois, classement et manches. Depuis le 2026-09-17, `/api/tournaments` renvoie un résumé par tournoi (`src/lib/tournament-overview.ts`) : état affiché dérivé des dates (`live`, `upcoming`, `finished`, `draft`), libellé de carte, nombre de manches, nombre de clans engagés et vainqueur calculé comme sur la page de classement |
