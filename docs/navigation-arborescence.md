...
- **Stockage :** clé sessionStorage `pubg-nav-stack`, tableau `{ href: string; label: string; ts: number }[]`, plafonné à 30 entrées (FIFO), déduplication si `href` identique à la dernière entrée poussée. *(Évolution du 2026-09-15 : la déduplication porte sur le **chemin sans la query** — une page qui change seulement ses paramètres d'URL, ou affine son libellé après chargement, met à jour sa dernière entrée au lieu d'en empiler une nouvelle. Logique pure et testée dans `src/lib/nav-stack.ts`. Une page dont l'état vit dans la query doit la passer dans `currentHref` : c'est le cas du comparateur `/clans/comparator?clanIds=…&period=…`, qui monte un `NavigationTrail` invisible (`hidden`) pour que le retour depuis un match restaure sa sélection.)* *(Évolution du 2026-09-16 : revenir sur une page **déjà présente** dans la pile la coupe à cette page, comme le bouton retour d'un navigateur — sans cela « Retour » empilait la page précédente et deux pages se renvoyaient l'une à l'autre sans fin (signalé entre `/tournaments/[id]` et le débriefing d'une manche). La coupe se fait à la première occurrence, ce qui nettoie aussi les piles polluées par l'ancien comportement. `/tournaments` s'inscrit désormais dans la pile via un `NavigationTrail` invisible.)*
