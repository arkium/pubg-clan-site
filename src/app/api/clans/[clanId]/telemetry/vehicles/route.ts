...
export async function GET(
  ...
  try {
    ...
    >(Prisma.sql`
      SELECT
        COUNT(*) AS snapshots,
        COALESCE(SUM(CAST(JSON_UNQUOTE(JSON_EXTRACT(t.summary, '$.vehicleEvents')) AS UNSIGNED)), 0) AS vehicleEvents
    ...
    const summary = rows[0] ?? { snapshots: 0, vehicleEvents: 0 }
    const snapshots = Number(summary.snapshots)
    const vehicleEvents = Number(summary.vehicleEvents)

    const vehicles = {
      snapshots,
      vehicleEvents,
      avgVehicleEventsPerMatch: snapshots > 0 ? Number((vehicleEvents / snapshots).toFixed(2)) : 0,
    }
    ...
    console.error('Telemetry vehicles failed:', error)
    return NextResponse.json(buildTelemetryErrorResponse('Failed to load telemetry vehicles'), {
      status: 500,
    })
  }
}
