...
import { WeaponStatsTable } from '@/app/clans/[clanId]/matches/[matchId]/telemetry/WeaponStatsTable'
...
export default function MatchTelemetryDetailPage() {
  ...
  const clanId = useMemo(() => parseId(searchParams.get('clanId') ?? undefined), [searchParams])
  ...
  const groupedMemberStats = useMemo(() => {
    ...
    return knownGroups.filter(group => group.members.some(m => !m.memberKey.toLowerCase().startsWith('ai.')))
  }, [memberStats, payload?.memberIdentityMap, clanId])

  const botsKilled = useMemo(() => {
    return memberStats
      .filter((m) => m.memberKey.toLowerCase().startsWith('ai.'))
      .reduce((sum, m) => sum + (m.deaths > 0 ? m.deaths : 0), 0)
  }, [memberStats])
        ...
        <>
          <section className="app-panel overflow-hidden p-0">
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
              <div className="app-table-shell p-4 flex flex-col items-center justify-center text-center">
                <span className="text-[10px] uppercase tracking-wider text-[var(--theme-ui-text-muted)] font-semibold">Carte</span>
                <span className="mt-1 text-base font-bold text-[var(--theme-ui-text)]">{match.mapName}</span>
              </div>
              <div className="app-table-shell p-4 flex flex-col items-center justify-center text-center">
                <span className="text-[10px] uppercase tracking-wider text-[var(--theme-ui-text-muted)] font-semibold">Mode</span>
                <span className="mt-1 text-base font-bold text-[var(--theme-ui-text)]">{resolveGameMode(match.gameMode)}</span>
              </div>
              <div className="app-table-shell p-4 flex flex-col items-center justify-center text-center">
                <span className="text-[10px] uppercase tracking-wider text-[var(--theme-ui-text-muted)] font-semibold">Date</span>
                <span className="mt-1 text-sm font-semibold text-[var(--theme-ui-text)]">{formatDateTime(match.createdAt)}</span>
              </div>
              <div className="app-table-shell p-4 flex flex-col items-center justify-center text-center bg-blue-50/50 dark:bg-blue-900/10 border-blue-100 dark:border-blue-900/30">
                <span className="text-[10px] uppercase tracking-wider text-blue-600/80 dark:text-blue-400 font-bold">Kills Globaux (Joueurs)</span>
                <span className="mt-1 text-2xl font-black text-blue-600 dark:text-blue-400">{summary?.killEvents ?? 0}</span>
              </div>
              <div className="app-table-shell p-4 flex flex-col items-center justify-center text-center bg-purple-50/50 dark:bg-purple-900/10 border-purple-100 dark:border-purple-900/30">
                <span className="text-[10px] uppercase tracking-wider text-purple-600/80 dark:text-purple-400 font-bold">Bots éliminés</span>
                <span className="mt-1 text-2xl font-black text-purple-600 dark:text-purple-400">{botsKilled}</span>
              </div>
            </div>
