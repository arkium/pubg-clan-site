...
export type SectionAnchorNavIcon =
  | 'playstyle'
  | 'combat'
  | 'victory'
  | 'support'
...
export type SectionAnchorNavItem = {
  id: string
  label: string
  icon?: SectionAnchorNavIcon
}

type SectionAnchorNavProps = {
  items: SectionAnchorNavItem[]
  ariaLabel: string
  className?: string
}

function renderIcon(icon: SectionAnchorNavIcon) {
  if (icon === 'combat') {
    return (
...
/** Distance sous le haut de l'écran à laquelle une section devient « active » : header + bandeau. */
function activationOffset() {
  const toolbar = parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--app-toolbar-height'))
  return readHeaderHeight() + (Number.isFinite(toolbar) ? toolbar : 0) + 24
}
...
export default function SectionAnchorNav({ items, ariaLabel, className }: SectionAnchorNavProps) {
  ...
  useEffect(() => {
    ...
    const syncWithHash = () => {
      ...
      if (itemIds.includes(nextId)) setActiveSectionId(nextId)
    }
    ...
    const syncWithScroll = () => {
      ...
      for (const item of items) {
        ...
        if (!section) continue
        if (section.getBoundingClientRect().top <= offset) nextId = item.id
        else break
      }
