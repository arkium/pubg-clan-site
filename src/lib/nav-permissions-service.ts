...
const PROMOTED_POSITIONS_KEY = 'nav_promoted_positions'
...
export async function getNavPromotedPositions(): Promise<Record<string, string[]>> {
  const config = await prisma.appConfig.findUnique({ where: { key: PROMOTED_POSITIONS_KEY } })
  return parsePositions(config?.value ?? null)
}

export async function setNavPromotedOrder(section: NavSection, orderedKeys: string[]): Promise<void> {
  const isValid = orderedKeys.every((k) => NAV_REGISTRY.some((i) => i.navKey === k))
  if (!isValid) throw new Error(`Invalid navKey in promoted order for section: ${section}`)

  const current = await getNavPromotedPositions()
  ...
  if (orderedKeys.length === 0) {
    ...
    await prisma.appConfig.upsert({
      where: { key: PROMOTED_POSITIONS_KEY },
      create: { key: PROMOTED_POSITIONS_KEY, value: JSON.stringify(rest) },
      update: { value: JSON.stringify(rest) },
    })
  } else {
    const updated = { ...current, [section]: orderedKeys }
    await prisma.appConfig.upsert({
      where: { key: PROMOTED_POSITIONS_KEY },
      create: { key: PROMOTED_POSITIONS_KEY, value: JSON.stringify(updated) },
      update: { value: JSON.stringify(updated) },
    })
  }
}
